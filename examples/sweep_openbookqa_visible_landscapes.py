...
def format_openbookqa_prompt(example: dict, answer_format: str) -> str:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sweep OpenBookQA examples to find visibly separated PCA landscapes.")
    parser.add_argument("--model-dir", type=Path, default=Path("/data/hansen/serv12/HRM-Deq/models/huginn-0125"))
    parser.add_argument("--output-path", type=Path, default=Path("outputs/rebuttal_ar_figures/openbookqa_visible_sweep.json"))
...
def main() -> None:
    ...
    print("[openbookqa-sweep] loading dataset...")
    ds = load_dataset(
        "openbookqa",
        "additional",
        split=f"{args.dataset_split}[{args.question_start}:{args.question_start + args.question_count}]",
    )
    print(f"[openbookqa-sweep] loaded {len(ds)} questions")

    print("[openbookqa-sweep] loading model...")
    ...
    model.eval()
    print("[openbookqa-sweep] model loaded")
    ...
    for local_idx, example in enumerate(ds):
        ...
        prompt_body = format_openbookqa_prompt(example, args.answer_format)
        print(f"[openbookqa-sweep] question={question_index}")
        messages = []
        ...
        for init_scale in init_scales:
            ...
            print(f"[openbookqa-sweep] question={question_index} init_scale={init_scale} candidates={len(search_candidates)}")
        ...
        print(f"[openbookqa-sweep] checkpoint wrote {args.output_path}")

    write_payload(all_results, completed_questions)
    print(f"[openbookqa-sweep] wrote {args.output_path}")
    if all_results:
        print("[openbookqa-sweep] best candidate:", json.dumps(all_results[0], ensure_ascii=False))
